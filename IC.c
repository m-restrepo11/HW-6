 ...
 *
 *      Este programa genera las condiciones iniciales (posiciones y velocidades) para que las 100 partículas
 *      de una galaxia de disco orbiten de manera estable en círculos alrededor de la masa central.
 ...
 *     Devuelve un archivo .dat que contiene las posiciones y velocidades de 120 partículas en orbita circular estable.
 */
...
int main(int argc, char **argv) {
	...
	FILE *datos;
	char *nombre = "condiciones_iniciales.dat";
	...
	for(i=0; i<12; i++){
		...
		fprintf(datos, "%d %f %f %f %f \n", i, x_array[i], y_array[i], vx_array[i], vy_array[i]);
		j++;
	}
	fclose(datos);
